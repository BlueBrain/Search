...
def test_run(get_es_client, model_response):
    ...
    for fd in fake_data:
        ...
        assert res["_source"]["ner_ml"][0]["entity"] == "cat"
        assert res["_source"]["ner_ml"][0]["score"] == 0.9439833760261536
        assert res["_source"]["ner_ml"][0]["entity_type"] == "ORGANISM"
        assert res["_source"]["ner_ml"][0]["start"] == 11
        assert res["_source"]["ner_ml"][0]["end"] == 14
        assert res["_source"]["ner_ml"][0]["source"] == "ml"

        assert res["_source"]["ner_ml"][1]["entity"] == "mouse"
        assert res["_source"]["ner_ml"][1]["score"] == 0.9975798726081848
        assert res["_source"]["ner_ml"][1]["entity_type"] == "ORGANISM"
        assert res["_source"]["ner_ml"][1]["start"] == 21
        assert res["_source"]["ner_ml"][1]["end"] == 26
        assert res["_source"]["ner_ml"][1]["source"] == "ml"

        assert res["_source"]["ner_ml_version"] == "v1"

        assert res["_source"]["ner_ruler_version"] is None

    # check that all paragraphs have been updated
    query = {"bool": {"must_not": {"term": {"field": {"ner_ml_version": "v1"}}}}}
    paragraph_count = client.count(index=index, query=query)["count"]
